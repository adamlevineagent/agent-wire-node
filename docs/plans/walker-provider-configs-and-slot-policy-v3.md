...
**2.16.1 Single-active-contribution invariant** `{invariant: config_contrib_active_unique}` `{txn: pyramid_config_contributions}` **(B-I5).** Today the `pyramid_config_contributions` table has no unique constraint enforcing one active contribution per `(slug, schema_type)`. Concurrent supersession (Settings save + ConfigSynced pull applying a Wire-side supersession on the same schema) can produce two rows with `status='active'` — resolver picks whichever `id DESC` returns, supersession chain is broken.

**Fix (Phase 0a):** add partial unique index `CREATE UNIQUE INDEX uq_config_contrib_active ON pyramid_config_contributions(slug, schema_type) WHERE status='active'`. Wrap `supersede_config_contribution` in `BEGIN IMMEDIATE TRANSACTION` to serialize on write intent. Second concurrent supersession fails with SQLITE_CONSTRAINT; caller retries or surfaces conflict to operator.
...
**Fix (§5.3 pre-migration check):** migration refuses to run if any row in `builds` has `status IN ('running','paused_for_resume')`. Boot-time modal surfaces: "Upgrade to v3 requires in-progress builds to finish or be marked failed. [Resume] [Mark failed] [Rollback to v2]". No silent data-corruption path.
...
**5.6.3 `use_chain_engine` default flip — PUNCHLIST P0-2 resolution.** The `PyramidConfig.use_chain_engine` flag currently defaults to `false`, meaning fresh installs bypass the chain engine entirely and hit the legacy `build.rs` pipeline that doesn't go through the Decision spine. Walker v3 is inert on such installs. **Phase 1 exit criterion** flips the `PyramidConfig::default()` to `true`. Existing-install migration handling (rev 1.0.1 fix for A-F5):

- **`use_chain_engine: true` already set:** no-op, pass through.
- **`use_chain_engine` absent or explicitly set to the old default `false`:** migration inspects and, if the value WAS the old default (distinguished via a one-time check against `bundled PyramidConfig::default()` at the time of their last save), rewrites to `true`. But if `false` was **explicitly operator-set** (detectable via a `user_set_use_chain_engine: bool` hint that gets stamped on any Settings save, OR conservatively: any `false` value in an existing file is treated as explicit), the boot-time migration modal (§2.17.3) surfaces: *"Walker v3 requires chain engine routing. Your install has chain engine disabled. [Enable + continue with walker v3] [Rollback to v2]."* Operator confirmation recorded as a `chain_engine_enable_ack` field on the `onboarding_state` contribution. No silent clobber.

This ties P0-1 + P0-2 together as a single Phase 1 outcome: legacy dispatch_policy routing path retires, chain engine becomes the only dispatch path, walker v3 is live on fresh installs AND operator-explicit `false` installs get a loud choice.
...
| **PUNCHLIST P0-1** (Ollama local builds fail immediately) | Resolved by §5.6.3 `use_chain_engine` flip + Phase 1 deletion of `config.primary_model` + migration §5.3 populating `walker_provider_openrouter.model_list`. Root cause (resolve_ir_model returns primary_model) eliminated when primary_model ceases to exist. |
| **PUNCHLIST P0-2** (`use_chain_engine` defaults to false → fresh installs bypass chain engine) | Resolved by §5.6.3. Jointly closed with P0-1 as a single Phase 1 outcome. |
