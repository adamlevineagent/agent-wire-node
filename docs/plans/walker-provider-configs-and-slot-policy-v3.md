...
**Date:** 2026-04-21
**Status:** DRAFT (rev 1.0.2) — post-fresh-audit plan-to-code reconciliation. Code-verified file/line references across §2.12, Phase 2, §5.1, §6 Phase 0b, §8, Phase 4.
**Rev:** 1.0.2
...
Concrete consumers for the synthetic path (rev 1.0.2 — retargeted against live code):
- **DADBEAR dispatch** — live dispatch is `src-tauri/src/pyramid/dadbear_supervisor.rs:514 dispatch_materialized_item`, which currently threads `DispatchOrigin::Local` through `LlmConfig::prepare_for_replay`. This is where the Decision integrates. `src-tauri/src/pyramid/stale_engine.rs` is **DECOMMISSIONED for dispatch** (explicit comments at lines 180, 498, 602, 619-622; `drain_and_dispatch` is a no-op). Do NOT target stale_engine as the synthetic-Decision site; its remaining role is timer/observation plumbing, not routing.
- **DADBEAR compile-time preview** — builds synthetic at compile time (pre-materialization), persists in preview payload. Apply-time `dispatch_materialized_item` builds a fresh full Decision; chronicle emits `preview_vs_apply_drift` if the two disagree on provider choice.
...
| `compute_participation_policy.market_saturation_patience_secs` (F-D7) | Absorbed into `walker_provider_market.overrides.patience_secs`. **Live touchpoint (rev 1.0.2 / code-verified):** read in the market dispatch retry loop at `src-tauri/src/pyramid/llm.rs:2216-2232` via `get_compute_participation_policy(&conn)` on a fresh SQLite connection; field flows through `market_saturation_patience_secs` local at `llm.rs:591/594/595/730/747/823/903`. Migration work: Decision must be in scope at the `llm.rs:2216` retry loop, replacing the fresh-connection CPP read with `decision.per_provider[market].patience_secs`. The remaining CPP surface (whole-node "am I participating at all" gate) stays — it's orthogonal to walker's per-slot `active` flag. Bundled-seed absorption on first boot. |
| `compute_participation_policy.market_dispatch_max_wait_ms` (F-D7) | Absorbed into `walker_provider_market.overrides.dispatch_deadline_grace_secs` (unit-converted). **Touchpoint:** same `llm.rs:2216-2232` block — the `market_max_wait_ms` local at `llm.rs:2216` is read from the same CPP fetch; Decision replaces this read too. |
| `resolve_ir_model` legacy tier→model mapping in `chain_dispatch.rs:1059` | **Subsume, don't rebuild (rev 1.0.2 / code-verified).** Line 1067 already consults `config.provider_registry.resolve_tier(tier, None, None, None)` as priority-2 in a Phase 3 fix-pass. Walker v3 replaces the priority-1 (`reqs.model`) + priority-2 (`registry.resolve_tier`) chain with `decision.per_provider[resolved_provider].model_list` reads. The already-migrated registry-consulting path is the foothold; the `match tier` fallback at lines 1080-1088 (still reading `primary_model`/`fallback_model_1`/`fallback_model_2`) is what actually retires. Framing: Phase 1 implementers should treat `resolve_ir_model` as a partial migration to complete, not a greenfield rewrite. |
| `resolve_tier_registry` in `yaml_renderer.rs:428` reading from `pyramid_tier_routing` (F-D5) | Phase 0 rewrites to compute the union of `model_list` keys across active walker_provider_* contributions. Same public API, new data source. Evidence_policy and any other schema_annotation using `options_from: tier_registry` continue to work without change. **Peer callers (rev 1.0.2 / code-verified):** `yaml_renderer.rs:575` and `:749` also call `registry.list_tier_routing()` directly. Phase 0 must treat them as peers of the :428 rewrite — updating the data source at :428 alone leaves the UI tier surface silently reading from `pyramid_tier_routing` via the other two callers. All three sites migrate together. |
| `build_runner.rs:328 / :358` live `use_chain_engine` reads (rev 1.0.2 / code-verified, not in v1 consumer inventory) | Absorbed into §5.6.3 chain-engine-enable-ack flow. The `from_depth` code path at `build_runner.rs:358` errors with "from_depth is only supported with the chain engine" — when `use_chain_engine: false` + chain-engine-enable-ack modal is surfaced, this branch is the concrete user-visible failure signal. Phase 1 adds this file to the consumer inventory explicitly. |
...
- **Envelope writer extraction** (~300–500 LOC, B-F1 / Root 13 resolution). Extract `write_contribution_envelope(schema_type, body, source, extras) -> Result<ContributionId>` as the sole `INSERT INTO pyramid_config_contributions` site. Refactor the **exactly 35** existing INSERT call sites across exactly 9 files (rev 1.0.2 verified count): `db.rs` (8), `config_contributions.rs` (9), `migration_config.rs` (5), `wire_migration.rs` (5), `generative_config.rs` (2), `wire_pull.rs` (2), `prompt_cache.rs` (2), `demand_signal.rs` (1), `evidence_answering.rs` (1). **Grep discipline:** the deny-rule MUST match `INSERT INTO pyramid_config_contributions` exactly — dropping the `pyramid_` prefix expands to 44 sites across 7 files on the different `config_contributions` table (not the walker target). The canonical production table is `pyramid_config_contributions`. Add the CI deny-rule with that exact string.
...
- **ProviderReadiness trait + four impls** (~200 LOC). Trait def per §2.6. Impls in `pyramid/local_mode.rs`, `pyramid/fleet_mps.rs` AND/OR `src-tauri/src/fleet.rs`, `pyramid/provider.rs` (openrouter), `pyramid/compute_market_ctx.rs` + `pyramid/compute_market_ops.rs` (market). Each impl ~30–80 LOC carrying the reason enum. **Module boundary note (rev 1.0.2 / code-verified):** `fleet.rs` lives at `src-tauri/src/fleet.rs` (top-level crate), while `fleet_mps.rs` is inside `src-tauri/src/pyramid/`. The `ProviderReadiness for Fleet` impl crosses this boundary — the trait itself lives in `pyramid/walker_resolver.rs` or `pyramid/mod.rs`; the impl block needs the correct `use` path. Not a blocker; a plan the implementer won't discover until cargo-check time if not named here.
...
- **Schema-registry 4-part completeness assertion (rev 1.0.2 / code-verified gap):** `schema_registry.rs` today does NOT enforce that every registered schema has all four parts (`schema_definition` + `schema_annotation` + `generation_skill` + `default_seed`); tests at `schema_registry.rs:655-670` explicitly exercise partial registration. Phase 0b adds `test_walker_schemas_four_part_complete` asserting that each of the six walker_* schemas (plus `migration_marker`, `onboarding_state`, `node_identity_history`, and the `compute_market_offer` skill) resolves all four parts at boot. Prevents the "Tools > Create card shows but no generation skill registered" failure mode visible in the Settings UI today.
...
- Ollama probe integration — **code reality (rev 1.0.2):** there is no `ollama_probe.rs` file. The probe is `pub async fn probe_ollama(base_url: &str) -> OllamaProbeResult` at `src-tauri/src/pyramid/local_mode.rs:330`, with live callers at `local_mode.rs:499`, `:570`, `:1204`. `ProviderReadiness for local` calls `probe_ollama()` directly from its impl; no file extraction required. If an implementer prefers to extract, it's a refactor-for-cleanliness not a missing dependency.
...
- Tests: Ollama probe (wrap `probe_ollama`), fallback-if-model-not-loaded, enabled-gate honored.
...
- **Tester smoke:** GPU-less tester installs app, asks question on small corpus. Bundled call-order `[market, local, openrouter, fleet]`. Decision at step entry: `market` ready (onboarding seeded ≥1 credit AND balance cache primed during onboarding Page 4 — see orchestration note below), `local` NotReady(OllamaOffline), `openrouter` NotReady(CredentialMissing), `fleet` NotReady(NoReachablePeer). `effective_call_order: [market]`. Network providers matching tester's market config serve via market. If market dry for the requested tier (`MarketSurfaceCache` has zero non-self offers) → `decision_build_failed` chronicle event with reason `NoMarketOffersForSlot`; build fails loud rather than silently burning patience on a dry market.
  - **Onboarding/walker orchestration (Root 10 / Issue 3, Root 13 F-D2, Root 17):** `tester-onboarding.md`'s Page 4 (tunnel validation) additionally: (a) primes the market balance cache via parallel `/balance` fetch alongside the existing handle-check, (b) flips `walker_provider_market.overrides.active` from the bundled `false` to `true` as the explicit consent supersession (operator-authored, chronicle-logged — this IS the market-participation consent record), (c) writes `onboarding_complete_at` timestamp to an `onboarding_state` contribution (rev 1.0 §2.18 converged here — the rev-0.9 "pyramid_config.json" residual was struck in rev 1.0.1 per B-F3). **Current verified owner:** the existing onboarding save path lives in `src-tauri/src/main.rs` as `save_onboarding(...)` at line 2978; `src-tauri/src/pyramid/onboarding.rs` does not exist today. Therefore Phase 0a-2 MUST either extend `main.rs::save_onboarding` directly to write the contribution, or land a prior/shared extraction that creates `src-tauri/src/pyramid/onboarding.rs` and moves ownership there. Do not leave this as an unnamed "(or equivalent)" write site.

**IPC signature gap (rev 1.0.2 / code-verified):** the current `save_onboarding` Tauri IPC takes 5 fields — `node_name`, `storage_cap_gb`, `mesh_hosting_enabled`, `auto_update_enabled`, `pin`. `onboarding_state` contribution holds a different 5-field set per §2.18 — `onboarding_complete_at`, `completed_pages`, `migration_acks`, `chain_engine_enable_ack`, `re_onboarding_required`. The existing IPC does not carry those; Phase 0a-2 must either (a) extend the `save_onboarding` signature to accept the contribution fields alongside or separately from the existing 5, or (b) add companion IPCs (e.g. `record_chain_engine_ack`, `record_migration_providers_ack`) that write to the same `onboarding_state` contribution. Option (b) is likely cleaner since the consent moments are temporally separate; signatures extending is fine if you prefer one surface. Either way this is named Phase 0a-2 work, not "trivial ~10 LOC." Walker plan and tester-onboarding plan must be rev-aligned on this dependency — the integrity-pass script (§2.13 item 6) enforces that `onboarding_complete_at` has a named writer. Boots where the timestamp is >5min old require a fresh balance fetch before market readiness passes.
...
- **Rev 1.0.2 (2026-04-22):** Fresh-eyes pair audit of rev 1.0.1 against live code. Two agents ran architectural-premise verification and consumer-inventory-plus-cross-cut verification; 12 annotations landed on `agent-wire-node2` pyramid (nodes L0-008, L0-113, L0-363, L0-375, L2-005, Q-L0-204). Arch audit returned 8/8 premises structurally green. Integration audit found 4 plan-to-code drifts worth fixing pre-Phase-0a-1:
  - **§2.12 retargeted (rev 1.0.2):** DADBEAR synthetic-Decision consumer was pointing at `stale_engine.rs:92-127`, but stale_engine is DECOMMISSIONED for dispatch (comments at lines 180/498/602/619-622; `drain_and_dispatch` is a no-op). Actual live DADBEAR dispatch is `dadbear_supervisor.rs:514 dispatch_materialized_item`. Retargeted.
  - **Phase 2 corrected (rev 1.0.2):** "`ollama_probe.rs` carries forward" described a file that doesn't exist. Actual probe is `pub async fn probe_ollama(base_url: &str)` at `local_mode.rs:330` with callers at `:499`, `:570`, `:1204`. Plan now references the function; extraction is optional, not a missing dependency.
  - **§5.1 absorption touchpoints named (rev 1.0.2):** `compute_participation_policy.market_saturation_patience_secs` and `.market_dispatch_max_wait_ms` are read at `llm.rs:2216-2232` via `get_compute_participation_policy(&conn)` on a fresh SQLite connection inside the market dispatch retry loop. Plan now names this as the migration site — the Decision must be in scope at that retry loop, replacing the fresh-connection read. Not a rename; a real threading refactor.
  - **§5.1 resolve_ir_model reframed "subsume, not rebuild" (rev 1.0.2):** `chain_dispatch.rs:1067` already consults `provider_registry.resolve_tier()` as priority-2 from Phase 3 fix-pass work. Walker v3 completes the partial migration; the `match tier { "low" | "mid" => primary_model, ... }` fallback at :1080-1088 is what actually retires. Framing matters for implementer mental model.
  - **Phase 0b 4-part completeness assertion added:** `schema_registry.rs` today does NOT enforce all-four-parts; tests at `:655-670` exercise partial registration. New `test_walker_schemas_four_part_complete` asserts six walker_* schemas + three new schemas + compute_market_offer skill all resolve their four parts at boot.
  - **§8 save_onboarding IPC signature gap named:** current IPC takes 5 operational fields (`node_name`, `storage_cap_gb`, `mesh_hosting_enabled`, `auto_update_enabled`, `pin`) — different from onboarding_state's 5 contribution fields. Phase 0a-2 extends signature OR adds companion IPCs. Not "trivial ~10 LOC" as previously assumed.
  - **§5.1 yaml_renderer peer callers named (rev 1.0.2):** `list_tier_routing()` has callers at `yaml_renderer.rs:575` and `:749` in addition to the :428 rewrite target. All three migrate together or UI tier surface silently drifts.
  - **§5.1 build_runner.rs:328/:358 added to retires table:** live `use_chain_engine` consumer + `from_depth` error path that becomes the user-visible signal for §5.6.3's chain-engine-enable-ack modal. Not in v1 consumer inventory.
  - **Phase 0a module-boundary note:** `ProviderReadiness for Fleet` impl crosses `src-tauri/src/fleet.rs` (top-level) and `src-tauri/src/pyramid/fleet_mps.rs` module boundary. Named in Phase 0a body.
  - **Envelope writer grep discipline:** `INSERT INTO pyramid_config_contributions` hits exactly 35 sites / 9 files (confirmed rev 1.0.2). Dropping the `pyramid_` prefix expands to 44 sites / 7 files on a different table. CI deny-rule MUST use the full table name.
  - **Pyramid annotations:** 12 landed on `agent-wire-node2`, making future audit rounds informable without re-reading the plan cold. Pyramid-first discipline now has real coverage on walker v3 nodes.
  - Plan is ready for Phase 0a-1 commit 1 pending baseline build-check.
