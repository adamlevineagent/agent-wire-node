...
### Post-ship finding (3) — 2026-04-21: walker pre-loop short-circuit bypasses market + fleet (walker-blocking for production route shape)

**Bug.** The compute-queue enqueue block at `llm.rs:1774-1878` (pre-`refactor(llm)`, now deleted) ran BEFORE the walker for-loop. Its gate used `should_enqueue_local_execution(route.providers.iter().any(|e| e.is_local))` which fires for ANY route containing an `is_local: true` entry. The bundled seed's `ollama-local` sits at route position 4 with `is_local: true`, so every outer dispatch short-circuited into the queue BEFORE the walker loop ran. Market + fleet + openrouter entries that preceded ollama-local in the route were unreachable: the inner walker (GPU-loop replay) runs `prepare_for_replay(Local)` which clears `compute_market_context`, so the market branch always emitted `no_market_context` and advanced. Fleet + openrouter replays likewise had their contexts stripped. In effect, `[market, fleet, openrouter, ollama-local]` always resolved on ollama-local — market and fleet were dead code in production.

**Fix (branch `fix/walker-queue-short-circuit-bypass`, this rev).** Move the enqueue INSIDE the walker's Pool branch, gated per-entry on `entry.is_local && config.compute_queue.is_some() && !options.skip_concurrency_gate && !walker_bypass_pool`. Market + fleet now run first; local pool falls through to the queue only when its walker turn arrives. GPU-loop Err classified as `CallTerminal` on the outer walker — the replay guard prevents other route entries from re-trying the local path. Deleted the now-unused `should_enqueue_local_execution` + `queue_model_id_for_local_execution` helpers. Three new regression tests: production-shape chronicle ordering, cascade-through-all-branches (market+fleet+openrouter+ollama-local), outer-call-reaches-market guard.

**Scope gap (walker-blocking for production route shape).** Unit tests used a convenient route shape (`['market', 'unknown-pool']`) that never triggers the short-circuit because `unknown-pool` has `is_local: false` by default in `walker_test_policy`. Production route shape (four entries including `is_local: true` on ollama-local) went untested. Wave 1 verifier + Wave 3 wanderer both saw `walker_resolved` fire and interpreted success as correctness. Acceptance criteria didn't include "market/fleet chronicle events fire when walker runs against a production route shape." Future walker work: every new walker test fixture either uses the bundled-seed shape or explicitly documents why its synthetic shape is equivalent. Added `walker_test_policy_with_local_pool` helper for the production shape.

**Subtler lesson.** `walker_resolved` firing is a confirmation that SOME entry resolved, not that the walker walked the route. The check that matters is "did each route entry get its branch's chronicle event?" — that's what catches a short-circuit upstream of the walker loop. The wanderer brief for a walker rebuild should list per-branch chronicle events as acceptance signals, not just the terminal `walker_resolved`.
