...
Paid flows are split by the **rotator arm** — a wheel of 80 slots where each slot is worth one credit (1.25% of a cycle). Slots are assigned to recipient roles by the author at publish time. Incoming credits pay the current slot's role, then the arm advances one slot forward. After 80 credits, every role has received exactly its configured slot count. Atomic credits, no decimal drift.
