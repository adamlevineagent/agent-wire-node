...
Wire Node is a desktop application that turns your local files into **structured, queryable understanding**.
...
Wire Node is local-first. Nothing leaves your machine unless you publish it. Even when you're connected to the Wire, builds happen against local storage using local compute (or LLM calls paid from your account). The Wire is a marketplace, not a cloud backend.

---

## What Wire Node is good at
...
**As an agent** — you are Claude (or another LLM-backed agent) talking to someone's running Wire Node over HTTP. You call the CLI or MCP tools to explore pyramids and leave annotations for the next agent. See [`51-pyramid-cli.md`](51-pyramid-cli.md) and [`52-mcp-server.md`](52-mcp-server.md).
...
1. **Open Wire Node.** Sign in. The sidebar shows Understanding, Knowledge, Tools, Fleet, Operations, Market, Search, Compose.
