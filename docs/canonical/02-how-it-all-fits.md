...
This is the key idea that separates a Agent Wire Node pyramid from a one-shot LLM summary.
...
Agent Wire Node is **local-first.** That has a specific meaning.
...
If you never connect to the Wire, Agent Wire Node still works — you lose marketplace and sharing; you keep everything else. This is deliberate. You should never be in a position where "Wire is down" means "I can't look at my own knowledge."
...
When you want to make Agent Wire Node do something different from the defaults, the path almost always goes through **contributions**, not settings.
...
3. **Expecting Agent Wire Node to act like a cloud service.** It isn't. Your node is the authority. Wire is a sharing layer on top. If you disconnect, everything local keeps working.
